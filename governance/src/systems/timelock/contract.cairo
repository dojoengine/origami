...
mod timelock {
    ...
    fn execute_transaction(
        world: IWorldDispatcher, target: ContractAddress, new_implementation: ClassHash, eta: u64
    ) {
        ...
        assert!(queued_tx.queued, "Timelock::execute_transaction: Transaction hasn't been queued.");
        ...
        assert!(
            timestamp <= eta + GRACE_PERIOD, "Timelock::execute_transaction: Transaction is stale."
        );
        set!(
            world,
            QueuedTransactions { contract: target, class_hash: new_implementation, queued: false }
        );
    ...
    fn que_transaction(
        ...
        set!(
            world,
            QueuedTransactions { contract: target, class_hash: new_implementation, queued: true }
        );
        emit!(
            world, timelockevents::QueueTransaction { target, class_hash: new_implementation, eta }
        );
    }
    ...
    fn cancel_transaction(
        ...
        emit!(
            world, timelockevents::CancelTransaction { target, class_hash: new_implementation, eta }
        );
    }
}
